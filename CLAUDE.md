...
service-worker.js       # precache shell + export libs, cache-first, "new version" update flow
...
CHANGELOG.md            # per-release "what's new" (feeds the in-app update toast text)
...
Note: the service worker uses `clients.claim()`, so the page reloads **only** on a user-accepted
update (`window.__acceptedUpdate` guard) — never on the first-load `controllerchange` (that was the
"flash on open" bug). Keep that guard if you touch the SW update flow.
