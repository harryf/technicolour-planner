...
export async function runUpgradeData() {
  ...
  s.ok("migrates to schema 3", after.schemaVersion === 3, `schema=${after.schemaVersion}`);
  ...
  s.ok("re-migrate is a no-op", again.schemaVersion === 3 &&
  ...
  s.ok("messy schema-2 file migrates without throwing", !threw && messyOut && messyOut.schemaVersion === 3, threw ? "threw" : "schema 3");
  ...
  s.ok("boot upgraded her file to schema 3", win2.eval("state.schemaVersion") === 3, `schema=${win2.eval("state.schemaVersion")}`);
